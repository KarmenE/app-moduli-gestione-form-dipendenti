...
//funzione per la autocompilazione dei campi con dati predefiniti
...
submitBtn.addEventListener ("click", function (e) {
    ...
    if (valid) {
        ...
        messagePar.innerHTML = "Uscita effettuata";  
        svuotaCampi();
    }
    else {
        messagePar.classList.remove("success");
        messagePar.classList.add("error");
        messagePar.innerHTML = "Errore disconnessione";
    }
...
//     document.getElementById('oraFine').value = currentDateTime;
// });

document.addEventListener('DOMContentLoaded', (event) => {
    const currentTime = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
    document.getElementById('oraFine').value = currentTime;
});
